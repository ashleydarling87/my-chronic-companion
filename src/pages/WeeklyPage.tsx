...
Generated by Buddy • ${format(new Date(), "MMM d, yyyy")}`;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
        ...
        <div className="mx-auto max-w-lg space-y-4">
          ...
          <div className="rounded-2xl bg-primary/10 p-4 animate-slide-up">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xl">{getBuddyEmoji(prefs?.buddy_avatar || "bear")}</span>
              <h2 className="text-sm font-bold text-primary">{prefs?.buddy_name || "Buddy"}'s Take</h2>
            </div>
          ...
          {/* PHQ-4 Mental Health Stats */}
          {(() => {
            const filteredMH = mhScores.filter((s) =>
              isWithinInterval(new Date(s.created_at), { start: startOfDay(startDate), end: endOfDay(endDate) })
            );
            if (filteredMH.length === 0) return null;
            const avgTotal = Math.round((filteredMH.reduce((s, m) => s + m.total_score, 0) / filteredMH.length) * 10) / 10;
            const avgAnx = Math.round((filteredMH.reduce((s, m) => s + (m.anxiety_score ?? 0), 0) / filteredMH.length) * 10) / 10;
            const avgDep = Math.round((filteredMH.reduce((s, m) => s + (m.depression_score ?? 0), 0) / filteredMH.length) * 10) / 10;
            const latestSeverity = filteredMH[0]?.severity || "normal";
            const sevColor = getSeverityColor(latestSeverity);
            
            const mhChartData = [...filteredMH]
              .sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())
              .map((s) => ({
                day: format(new Date(s.created_at), "MMM d"),
                anxiety: s.anxiety_score ?? 0,
                depression: s.depression_score ?? 0,
                total: s.total_score,
              }));
              ...
              <>
                <div className="rounded-2xl border bg-card p-4 animate-slide-up">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-sm font-bold">Mental Health (PHQ-4)</h3>
                    <span className={`ml-auto rounded-full px-2.5 py-0.5 text-[10px] font-semibold capitalize ${sevColor}`}>
                      {latestSeverity}
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    <div className="rounded-xl bg-secondary/50 p-2.5 text-center">
                      <p className="text-lg font-bold">{avgTotal}</p>
                      <p className="text-[10px] text-muted-foreground">Avg Total /12</p>
                    </div>
                    <div className="rounded-xl bg-secondary/50 p-2.5 text-center">
                      <p className="text-lg font-bold">{avgAnx}</p>
                      <p className="text-[10px] text-muted-foreground">Anxiety /6</p>
                    </div>
                    <div className="rounded-xl bg-secondary/50 p-2.5 text-center">
                      <p className="text-lg font-bold">{avgDep}</p>
                      <p className="text-[10px] text-muted-foreground">Depression /6</p>
                    </div>
                  </div>
                  {mhChartData.length > 1 && (
                    <>
                      <ResponsiveContainer width="100%" height={140}>
                        <LineChart data={mhChartData}>
                          <XAxis dataKey="day" tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" />
                          <YAxis domain={[0, 12]} tick={{ fontSize: 10 }} stroke="hsl(var(--muted-foreground))" />
                          <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "12px", fontSize: "12px" }} />
                          <Line type="monotone" dataKey="anxiety" stroke="hsl(var(--pain-mid))" strokeWidth={2} dot={{ r: 3 }} name="Anxiety" />
                          <Line type="monotone" dataKey="depression" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 3 }} name="Depression" />
                        </LineChart>
                      </ResponsiveContainer>
                      <div className="mt-2 flex items-center justify-center gap-4 text-xs">
                        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-pain-mid" /> Anxiety</span>
                        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-primary" /> Depression</span>
                      </div>
                    </>
                  )}
                  <p className="text-[10px] text-muted-foreground mt-2 text-center">{filteredMH.length} check-in(s) this period</p>
                </div>
              </>
            );
          })()}
      ...
      {/* Generated Report Full-Screen Sheet */}
      <Sheet open={showReportSheet} onOpenChange={(open) => {
        ...
        <SheetContent side="bottom" className="h-[95vh] rounded-t-2xl flex flex-col p-0">
          <div className="flex items-center justify-between px-4 pt-4 pb-2 border-b">
            <h3 className="text-base font-bold">Your Report</h3>
            <div className="flex items-center gap-2">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3">
            <Textarea
              ...
              className="min-h-full w-full border-0 bg-transparent text-xs font-mono leading-relaxed focus-visible:ring-0 focus-visible:ring-offset-0 resize-none"
            />
          </div>
          <div className="px-4 py-3 border-t">
            <Button onClick={handleSaveReport} className="w-full rounded-xl">Save Report</Button>
          </div>
        </SheetContent>
      </Sheet>
